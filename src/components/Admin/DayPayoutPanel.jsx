                ...
                <div className="px-6 py-6 space-y-6">
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 p-5 bg-[var(--color-surface-muted)]/50 border border-[var(--color-line)] rounded-[var(--radius-md)]">
                        <StatColumn label="Inputs">
                            <StatLine
                                label="Team Sales"
                                value={fmt(summary.derivedValues?.totalTeamSales)}
                                strong
                            />
                        ...
                        <StatColumn label="Balances">
                            <div className="flex items-baseline justify-between gap-3">
                                ...
                                >
                                    {fmt(summary.balances?.overallBalance)}
                                </dd>
                            </div>
                            <StatLine
                                label={`Available CTP (${fmt(summary.derivedValues?.ctpTotal)}) + GRT (${fmt(summary.derivedValues?.grtTotal)})`}
                                value={fmt((summary.balances?.totalAvailable || 0) - (summary.derivedValues?.baseTeamCash || 0))}
                            />
                            <StatLine
                                label="Distributed CTP + GRT"
                                value={fmt((summary.balances?.totalDistributed || 0) - (summary.derivedValues?.baseTeamCash || 0))}
                            />
                            <div className="mt-1 pt-2 border-t border-[var(--color-line)]">
                                <StatLine
                                    label="Total Team Points"
                                    value={summary.pointTotals?.totalAllTeamPoints || 0}
                                    strong
                                />
                            </div>
                        </StatColumn>
